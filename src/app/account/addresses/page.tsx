...
const inputClass = "w-full border border-[#e8e2db] px-4 py-2.5 text-[12px] text-[#1a1008] outline-none focus:border-[#1a1008] transition-colors font-serif placeholder:text-[#c8c0b8] bg-white";
  ...
  return (
    <div className="min-h-screen bg-[#f7f5f2] font-serif">
      <div className="max-w-2xl mx-auto px-5 py-12">
        ...
        {loading ? (
          <div className="text-[11px] tracking-widest text-[#8a7a6a]">Loading…</div>
        ) : (
          <>
                ...
                <div key={addr.id} className="bg-white border border-[#e8e2db] p-5 flex items-start justify-between gap-4">
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      <p className="text-[12px] text-[#1a1008]">{addr.fullName}</p>
                      {addr.isDefault && <span className="px-2 py-0.5 text-[9px] tracking-wide bg-[#1a1008] text-white rounded-full">Default</span>}
                    </div>
                  ...
                  <button onClick={() => handleDelete(addr.id)} className="text-[10px] text-[#8a7a6a] hover:text-red-600 underline underline-offset-2 transition-colors flex-shrink-0">Delete</button>
                </div>
              ))}
              {addresses.length === 0 && !showForm && (
                <div className="bg-white border border-[#e8e2db] p-10 text-center">
                  <p className="text-[12px] text-[#8a7a6a]">No saved addresses yet.</p>
                </div>
              )}
            </div>

            {!showForm ? (
              <button onClick={() => setShowForm(true)} className="border border-[#1a1008] px-6 py-2.5 text-[10.5px] tracking-[0.15em] uppercase text-[#1a1008] hover:bg-[#1a1008] hover:text-white transition-colors">
                + Add New Address
              </button>
            ) : (
              <div className="bg-white border border-[#e8e2db] p-6">
                <h2 className="text-[11px] tracking-[0.2em] text-[#8a7a6a] uppercase mb-5">New Address</h2>
                {error && <div className="mb-4 px-4 py-3 bg-red-50 border border-red-200 text-[11.5px] text-red-700">{error}</div>}
                <form onSubmit={handleSave} className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-[10px] tracking-[0.2em] text-[#8a7a6a] uppercase mb-1.5">Full Name</label>
                    ...
                    <div>
                      <label className="block text-[10px] tracking-[0.2em] text-[#8a7a6a] uppercase mb-1.5">Phone Number</label>
                  ...
                  <div>
                    <label className="block text-[10px] tracking-[0.2em] text-[#8a7a6a] uppercase mb-1.5">Address Line 1</label>
                  ...
                  <div>
                    <label className="block text-[10px] tracking-[0.2em] text-[#8a7a6a] uppercase mb-1.5">Address Line 2 <span className="text-[#c8c0b8] normal-case tracking-normal">(optional)</span></label>
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    <div>
                      <label className="block text-[10px] tracking-[0.2em] text-[#8a7a6a] uppercase mb-1.5">City</label>
                      <input type="text" value={form.city} onChange={e => set("city", e.target.value)} required placeholder="Lagos" className={inputClass} />
                    </div>
                    <div>
                      <label className="block text-[10px] tracking-[0.2em] text-[#8a7a6a] uppercase mb-1.5">State</label>
                    ...
                    <div>
                      <label className="block text-[10px] tracking-[0.2em] text-[#8a7a6a] uppercase mb-1.5">Country</label>
                  ...
                  <label className="flex items-center gap-2 cursor-pointer">
                    ...
                    <span className="text-[11px] tracking-wide text-[#3a2e22]">Set as default address</span>
                  </label>
                  <div className="flex gap-3 pt-2">
                    <button type="submit" disabled={saving} className="bg-[#1a1008] text-white text-[10.5px] tracking-[0.15em] uppercase px-6 py-2.5 hover:bg-[#3a2e22] transition-colors disabled:opacity-50">
                      {saving ? "Saving…" : "Save Address"}
                    </button>
                    <button type="button" onClick={() => { setShowForm(false); setError(null); setForm(emptyForm); }}
                      className="border border-[#e8e2db] text-[10.5px] tracking-[0.15em] uppercase px-6 py-2.5 text-[#8a7a6a] hover:border-[#1a1008] transition-colors">
                      Cancel
                    </button>
                  </div>
                </form>
