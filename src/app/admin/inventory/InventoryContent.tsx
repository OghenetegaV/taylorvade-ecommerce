...
type Product = {
  ...
  gender: string; isPublished: boolean;
  images: { url: string }[];
  variants: Variant[];
...
export default function InventoryContent() {
  ...
  async function saveStock(variantId: string, value: string) {
    ...
    if (isNaN(qty) || qty < 0) { alert("Enter a valid quantity (0 or more)"); return; }
    setSaving(true);
      ...
      setProducts((prev) =>
        prev.map((p) => ({
          ...
          variants: p.variants.map((v) =>
            v.id === variantId ? { ...v, stockQuantity: data.data.stockQuantity } : v
          ),
        }))
      );
      setEditing(null);
    ...
    <div className="p-8">
      ...
      <div className="flex items-center gap-3 mb-5 flex-wrap">
        <input type="text" placeholder="Search by name, SKU…" value={search}
          ...
          className="border border-[#e8e2db] bg-white px-4 py-2 text-[11.5px] outline-none focus:border-[#1a1008] w-56 font-serif"/>
        {[{ v:"all",l:"All" },{ v:"low",l:"Low Stock" },{ v:"out",l:"Out of Stock" }].map((f) => (
          <button key={f.v} onClick={() => { setFilter(f.v); setPage(1); }}
            className={`px-4 py-2 text-[10.5px] tracking-[0.1em] border transition-colors font-serif ${
              filter === f.v
      ...
      {loading ? (
        ...
        <div className="py-16 text-center text-[11px] tracking-widest text-[#8a7a6a]">No products found</div>
      ) : (
              ...
              <div className="flex items-center gap-4 px-5 py-4 border-b border-[#f0eeeb]">
                {product.images[0] && (
                  <div className="relative w-10 h-14 flex-shrink-0 overflow-hidden bg-[#f0eeeb]">
                    <Image src={product.images[0].url} alt={product.name} fill className="object-cover object-top" sizes="40px"/>
                  </div>
                )}
                ...
                <div className="flex items-center gap-4 text-right flex-shrink-0">
                  <div>
                    <p className="text-[10px] tracking-[0.1em] text-[#8a7a6a] uppercase">Total Stock</p>
                  ...
                  {!product.isPublished && (
                    <span className="px-2 py-0.5 text-[9.5px] bg-gray-100 text-gray-600 rounded-full">draft</span>
                  )}
                </div>
              </div>
                  ...
                  {product.variants.map((variant) => (
                    <tr key={variant.id} className="border-t border-[#f5f3f0]">
                      ...
                      <td className="px-5 py-2.5">
                        <span className={`px-2 py-0.5 rounded-full text-[10px] ${stockBadge(variant.stockQuantity)}`}>
                      ...
                      <td className="px-5 py-2.5">
                        {editing?.variantId === variant.id ? (
                          <div className="flex items-center gap-2">
                            ...
                            <button disabled={saving} onClick={() => saveStock(variant.id, editing.value)}
                          ...
                          <button onClick={() => setEditing({ variantId: variant.id, value: String(variant.stockQuantity) })}
