    ...
    <section className="bg-[#FAF9F7] py-14 md:py-20">
      <ScrollReveal>
        <div className="flex items-end justify-between px-5 md:px-12 mb-6 md:mb-8">
          <div>
            <p className="text-[10px] tracking-[0.3em] uppercase font-serif text-[#9a8a7a] mb-2">
              Just landed
            </p>
            <h2 className="text-[#1a1008] leading-none"
              style={{ fontFamily: "var(--font-script), cursive", fontSize: "clamp(28px,4vw,44px)" }}>
              Fresh off the table
            </h2>
          </div>
          <Link href="/collections/woman"
            className="text-[10px] tracking-[0.2em] uppercase font-serif text-[#5a4a3a]
              border-b border-[#5a4a3a]/40 pb-1 hover:border-[#1a1008] hover:text-[#1a1008]
      ...
      <div className="flex gap-3 md:gap-4 overflow-x-auto px-5 md:px-12 pb-4
        ...
        {products.map((product, i) => (
          ...
          <Link href={`/products/${product.slug}`} className="group block">
            <div className="relative overflow-hidden bg-[#eceae5]" style={{ aspectRatio: "2/3" }}>
              {product.images[0] && (
              ...
              {product.isNew && (
                <span className="absolute top-2.5 left-2.5 text-[9px] italic font-serif
                  text-white bg-[#1a1008]/85 px-2 py-0.5">
                  New In
                </span>
              )}
            </div>
            <div className="pt-2.5">
              <p className="text-[14px] text-[#1a1008] leading-tight truncate"
                style={{ fontFamily: "var(--font-script), cursive" }}>
                {product.name}
              </p>
              <p className="text-[10.5px] text-[#8a7a6a] font-serif mt-0.5 truncate">{product.type}</p>
              <p className="text-[11px] text-[#3a2e22] font-serif mt-0.5">
